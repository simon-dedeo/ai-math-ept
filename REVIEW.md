...
| lexical consolidation crossover: AI has more once-referenced claims, humans more claims with two or more exact-name references (all 12 source groups; persists under length and character controls); absolute multi-consumer-site supply is denominator-dependent | `code/paired_horizon.py` | `results/horizon/source_summary.json` → `claim_supply_per_100_tokens`, `claim_supply_per_1000_characters`, `claim_supply_length_matched`, `multi_consumer_site_claim_share` |
