...
| # | Hypothesis | Status |
|---|---|---|
| H1 | AI proofs reuse less than human proofs of comparable content | **Not supported where it is best tested.** At matched statement (§5g) distinct premises are *equivalent* (median diff 0, CI [0,0]). At proof-term level AI tails are *heavier* (§2). Observationally, AI corpora show lower vocabulary ratios (§5e, corpus-level p = 0.011 post-fix) but that design cannot separate authorship from domain. |
...
| H3 | Subgoal-decomposition provers are structurally closer to humans than whole-proof provers | **Not tested adequately.** Architecture is confounded with corpus identity and problem difficulty (§5f); the matched design that could test it yields a 27×3 block, too small to separate architecture families. Recorded as open, not as a result. |
| H4 | AI proofs are longer with lower per-step information | **Partly supported, and the "longer" half is corpus-specific.** In the Gauss/sphere-packing source comparison, 2× length and 4× `have` steps (§3). At matched statement, length differs by only +2 lines (inside a ±10% equivalence bound) while `have` steps still double (§5g). The robust part is the finer decomposition, not the length. |
...
| H6 | Verified corpora tolerate cross-module density / abandon firewalls | **Withdrawn as stated.** The ETP skeleton's path redundancy is *not* zero (mean 1.04 edge-disjoint paths) and is statistically indistinguishable from classical Coq proof DAGs (§1b, §5a). What is true is narrower: the ETP retained 0.13% of the one-step certificates available to it — a storage choice, not an absence of redundancy. |
| H8 | Machines eat the head of the difficulty distribution, humans keep the tail | **Provisional, and selection-conditioned.** Study 6's 5× load ratio rests on 20 human edges, uses one arbitrary Dijkstra predecessor path rather than all-shortest-path betweenness, and — decisively — the human-proved implications were *chosen by humans as structurally important*, so the result is evidence of curation, not of an independent division of labour. The §4b claim ("all true implications one-shot ATP-provable") comes from a log; the run must be repeated before it is quoted. |
