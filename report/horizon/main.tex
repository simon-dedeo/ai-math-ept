...
Lemma invention itself is not new.  Mining unnamed HOL Light inferences showed that useful
intermediate statements, combined with relevance filtering, can strengthen later proving even when
millions of candidates exist \citep{kaliszyk2014}.  LEGO-Prover grows a verified skill library
during theorem proving \citep{wang2023lego}; DreamProver alternates a wake phase that proposes
lemmas with a sleep phase that abstracts and consolidates them \citep{zhang2026dream}.  These systems
make the retrieval cost visible: persistent memory is useful only if a later agent can select from
it.  Our contribution is not the observation that lemmas can be learned, but a common account of
why compile-first artifacts, refactored proofs, human-readable boundaries, and growing libraries
differ---and an evaluation target that charges construction against future savings.
...
\begin{equation}
 ...
 \label{eq:apc}
\end{equation}
subject to every declaration in $L$ and every $P_t$ checking in the base environment, with
$\lambda\geq 1$.  The factor $\lambda$ prices the design and maintenance of shared structure.  A richer version can price names,
