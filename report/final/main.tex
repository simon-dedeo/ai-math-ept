...
The recovered convention has a small-network qualification.  The four smallest published Coq
networks reproduce jointly in exponent and standard error at $\xmin=5$, not 10.  No surviving
script contains that branch, suggesting an earlier or uncommitted small-network exception.  The
fixed-$\xmin=10$ comparison below remains well defined because it applies one declared estimand
uniformly across the recovered Coq and Lean samples.

The later Python analysis used the standard \code{powerlaw} behavior of choosing $\xmin$ to minimize
a KS distance.  It often chose a cutoff near one.  Fixed $\xmin=10$ reproduces 40 of 44 estimable
archived exponents within 0.010; automatic estimates have essentially no cross-network correlation with the
